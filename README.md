...
- Computes conformal identifiability screens: distribution-free lower bounds on future-window top-k ranking gaps over exchangeable stream windows, budget certification at level 1-alpha, held-out coverage checks, and adaptive conformal inference under drift (`graphsense/conformal.py`, `scripts/conformal_certificate.py`, `make conformal-certificate`).
- Prices certification in memory (Algorithm 1): the memory-minimal (width, capacity) that certifies future top-k identifiability, or `impossible` (`graphsense/pricing.py`, `make budget-pricing`); measured recall is 1.0 at the priced budgets.
- Evaluates with an identifiability-aware Recall@k whose budget-matched core reports `not identifiable` instead of a misleading zero on all-tie streams (`graphsense/metrics.py`, `make identifiable-recall`).
- Compares two heavy-hitter decoder families -- SpaceSaving candidate tracking and dyadic Count-Min tree descent -- across all regimes, confirming that discovery is bounded by signal structure, not data-structure choice (`graphsense/dyadic.py`, `make dyadic-comparison`).
- Adapts real captures to the pipeline (`scripts/adapt_real_trace.py`); the paper uses CTU-13 scenario 42 (2.8M bidirectional flows, addresses relabeled to opaque integers).
