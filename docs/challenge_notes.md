...
The public GraphChallenge data page lists Anonymized Network Sensing resources: a Random PCAP archive and a Random GraphBLAS archive in the public `graphchallenge` S3 bucket (organizer-generated synthetic traffic in official formats), plus CAIDA PCAP access links (access-controlled, not used). The full archives are 8-14 GB, so they are not downloaded in full. Instead, `scripts/fetch_official_prefix.py` Range-downloads byte-range prefixes of the official `pcap.zst` (256 MiB and 2 GiB), stream-decompresses the truncated zstd frame, and parses 5,000,000-40,000,000 fixed-size packet records into an anonymized `src,dst,bytes` edge table with a committed provenance manifest. `scripts/grb_cross_validation.py` verifies the parser exactly against the official GraphBLAS matrices (64/64 windows on the first 2^23-packet segment), and `scripts/benchmark_graphblas.py` adds a SuiteSparse:GraphBLAS construction baseline. These official-product prefixes validate the pcap parser, the early-stream method selector, the safety screen, and the candidate-capacity findings on organizer-published official data. They are synthetic official products; no claim is made about real telescope traffic.
